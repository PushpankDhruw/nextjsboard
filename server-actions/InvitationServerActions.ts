...
export async function handleSendBoardInvitation({
  ...
  try {
    ...
    const baseUrl = process.env.AUTH_URL;  // Use globalThis for environment variables
...
export async function handleRejectInvitation({
  ...
  const data = { token, userId };
  const parse = RejectInvitationSchema.safeParse(data);
  ...
  try {
    ...
    // Delete the invitation without accepting it
    await prisma.invitation.delete({ where: { id: invitation.id } });
